...
已有未保存看盘结果时，“新建分析”会先调用浏览器原生确认；取消后保留当前结果。在支持且已有用户交互的浏览器中，刷新、关闭标签页或离开站点也会触发浏览器原生离开保护。结果保存到档案后，两类保护都会解除，页面不常驻 `beforeunload` 监听。
...
- `npm run test:web`：8 个前端测试文件、49 个 Vitest 用例通过；
...
130 个通用 runner 用例、5 个 Pages 专项用例与 407 项权威矩阵口径不同：它们分别验证代码/组件、静态发布产物、原始测试矩阵 ID，不能相加为一个测试总数。
