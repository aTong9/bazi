...
三种模式最终都提交已经排好并由用户复核的四柱。每张命盘可手动选择四柱，或用已换算成固定 UTC+8 的公历时间辅助填入；后者不处理出生地时区、历史夏令时或真太阳时，边界时不会自动选盘。关系画像与现实评估可以选择传统夫妻星计算口径，并可选录入另一方命盘；另一方命盘只进入 `structuralSupplement`，不生成现实适配分数，也不替代现实行为、同意、安全事实或现实闸门。
