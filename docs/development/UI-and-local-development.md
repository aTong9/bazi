...
- `ArchivePanel.vue`：本地看盘档案按名称、称呼或四柱搜索，以及恢复、重命名、可取消删除确认、模态焦点循环与关闭后焦点恢复；
...
- `npm run test:web`：8 个前端测试文件、55 个 Vitest 用例通过；
...
136 个通用 runner 用例、5 个 Pages 专项用例与 407 项权威矩阵口径不同：它们分别验证代码/组件、静态发布产物、原始测试矩阵 ID，不能相加为一个测试总数。
