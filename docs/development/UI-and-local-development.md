...
已有未保存看盘结果时，“新建分析”会先调用浏览器原生确认；取消后保留当前结果。已保存到档案的结果可直接新建，不重复打断。
...
- `npm run test:web`：8 个前端测试文件、48 个 Vitest 用例通过；
...
129 个通用 runner 用例、5 个 Pages 专项用例与 407 项权威矩阵口径不同：它们分别验证代码/组件、静态发布产物、原始测试矩阵 ID，不能相加为一个测试总数。
