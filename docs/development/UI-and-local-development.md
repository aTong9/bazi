...
“下载可读摘要”生成 UTF-8 Markdown，包含本次命盘摘要、手动四柱或公历辅助来源、UTC+8 时间与历法适配器版本、分析 ID、完整规则快照摘要、证据等级、已裁决报告章节、下一步观察和阅读边界；`safety_stop` 时只导出安全章节和硬边界。“下载完整 JSON”生成 `bazi.relationship.reading.v1` 看盘包，包含导出时间、敏感数据标记、完整输入工作区、输入指纹和原始响应，供独立追踪使用；下载不会自动保存到浏览器档案。两种文件都可能包含敏感出生资料，均不加密。
