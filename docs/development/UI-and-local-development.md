...
“下载可读摘要”生成 UTF-8 Markdown，包含本次命盘摘要、手动四柱或公历辅助来源、UTC+8 时间与历法适配器版本、分析 ID、完整规则快照摘要、证据等级、已裁决报告章节、下一步观察和阅读边界；现实评估还包含八道现实闸门的裁决状态、已填写的事实依据及已发布的跨情境核验。`safety_stop` 时只导出安全章节和硬边界，不带出普通闸门或跨情境内容。“下载完整 JSON”生成 `bazi.relationship.reading.v1` 看盘包，包含导出时间、敏感数据标记、完整输入工作区、输入指纹和原始响应，供独立追踪使用；下载不会自动保存到浏览器档案，也可由“看盘档案”的同一导入入口校验、预览并恢复为单份档案。两种文件都可能包含敏感出生资料，均不加密。
