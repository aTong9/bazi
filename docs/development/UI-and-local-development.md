...
“下载可读摘要”生成 UTF-8 Markdown，包含本次命盘摘要、手动四柱或公历辅助来源、UTC+8 时间与历法适配器版本、分析 ID、完整规则快照摘要、证据等级、已裁决报告章节、下一步观察和阅读边界；`safety_stop` 时只导出安全章节和硬边界。“下载完整 JSON”保留原始响应，供技术追踪使用。两种文件都可能包含敏感出生资料，均不加密。
