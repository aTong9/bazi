...
同源页面之间监听原生 `storage` 事件：另一标签页保存、重命名、删除或导入档案后，当前页面会重新从经过校验的本地存储加载列表；若档案面板已打开，会显示同步提示。当前工作区不会被后台覆盖。外部删除当前档案后保存按钮恢复可用；外部修改同 ID 档案工作区、分析结果或规则版本后显示“更新档案”，覆盖前必须明确确认，取消则保留较新版本并提示先打开它。只读加载遇到损坏、超量、重复 ID 或结果身份错配的存储时降级为空列表，但保存、删除、重命名和导入会停止写入并提示先保全浏览器存储，避免把仍可恢复的原始数据静默覆盖。
...
- `npm run test:web`：8 个前端测试文件、56 个 Vitest 用例通过；
...
137 个通用 runner 用例、5 个 Pages 专项用例与 407 项权威矩阵口径不同：它们分别验证代码/组件、静态发布产物、原始测试矩阵 ID，不能相加为一个测试总数。
