...
三种模式的结果保存按钮都按同一安全基线显示“保存到档案”“已保存到档案”或“更新档案”。已保存状态禁用重复写入；保存后修改命盘或继续补充独立观察时，按钮自动恢复为可用的“更新档案”。M0 档案使用 `archive-m0-<requestId>`，关系档案继续使用旧有 `archive-<requestId>`，避免跨模式身份冲突且不迁移旧档案。用户重命名过的档案带有显式标记，更新工作区与备份往返都保留自定义名称；未重命名的旧档案继续根据命盘称呼生成标题。
...
145 个通用 runner 用例、6 个 Pages 专项用例与 407 项权威矩阵口径不同：它们分别验证代码/组件、静态发布产物、原始测试矩阵 ID，不能相加为一个测试总数。
