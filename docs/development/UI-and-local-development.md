...
同源页面之间监听原生 `storage` 事件：另一标签页保存、重命名、删除或导入档案后，当前页面会重新从经过校验的本地存储加载列表；若档案面板已打开，会显示同步提示。当前工作区不会被后台覆盖。外部删除当前档案后保存按钮恢复可用；外部修改同 ID 档案工作区、分析结果或规则版本后显示“更新档案”，覆盖前必须明确确认，取消则保留较新版本并提示先打开它。新档案同时保存生成结果时的输入指纹，导入与读取会拒绝输入、传统角色口径和结果被拼接错配的记录；旧档案在通过原有校验后补齐该指纹。只读加载遇到损坏、超量、重复 ID 或结果身份错配的存储时降级为空列表，但保存、删除、重命名和导入会停止写入，档案面板提供原始存储下载，并只在二次确认后清除损坏数据，避免把仍可恢复的内容静默覆盖。
