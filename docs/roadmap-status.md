...
| 5 正式报告 | Functional deterministic MVP plus optional AI adapter | `report-engine.ts`, `/api/chart`, report UI; selectable time/day/qiyun bases, visible calculation trace, local save/delete/export; `/api/narrative` sends only derived chapters with `store: false`, then rejects changed evidence or novel prose; safety and integration tests | Live provider smoke test requires operator-owned credentials; authenticated server persistence/share is not enabled |
