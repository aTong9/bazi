...
async function route(
  ...
  if (request.method === "POST" && url.pathname === "/v1/m0/analyze") {
    ...
    if (!body.ok) {
      sendJson(response, body.status, { issues: [{ code: body.code, severity: "error", stage: "request", message: body.message, retryable: false }] });
      return;
    }
...
async function readJsonBody(request: IncomingMessage): Promise<
  ...
  { ok: false; status: 400 | 413 | 415; code: "E_JSON" | "E_PAYLOAD_TOO_LARGE" | "E_CONTENT_TYPE"; message: string }
> {
  if (request.headers["content-type"]?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") {
    request.resume();
    return { ok: false, status: 415, code: "E_CONTENT_TYPE", message: "Content-Type must be application/json" };
  }
  const chunks: Buffer[] = [];
  let size = 0;
  for await (const chunk of request) {
    ...
    if (size > 1_048_576) {
      request.resume();
      return { ok: false, status: 413, code: "E_PAYLOAD_TOO_LARGE", message: "Request body exceeds 1 MiB" };
    }
    chunks.push(buffer);
  }
  try {
    ...
    return { ok: false, status: 400, code: "E_JSON", message: "Request body is not valid JSON" };
  }
}
