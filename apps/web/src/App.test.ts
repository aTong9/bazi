...
describe("App analysis provenance", () => {
  ...
  it("exports unreadable archive storage before allowing a confirmed reset", async () => {
    ...
    findButton(document.body, "清除损坏数据").click();
    await flushUi();
    localStorage.setItem(ARCHIVE_STORAGE_KEY, "new damaged archive bytes");
    findButton(document.body, "确认清除").click();
    await flushUi();
    expect(localStorage.getItem(ARCHIVE_STORAGE_KEY)).toBe("new damaged archive bytes");
    expect(document.body.textContent).toContain("已在另一标签页变化，未执行清理");
    findButton(document.body, "清除损坏数据").click();
    await flushUi();
    findButton(document.body, "确认清除").click();
    await flushUi();
  ...
  it("downloads a readable safety summary without ordinary report content", async () => {
    ...
    expect(summary).toContain("主要命盘资料状态：出生时间：准确；资料质量：高 · 已核对");
  ...
  it("syncs another tab and confirms before overwriting its newer workspace", async () => {
    ...
    vi.stubGlobal("confirm", confirmRestore);
    findButton(document.body, "打开档案").click();
    await flushUi();
    expect(confirmRestore).not.toHaveBeenCalled();
    expect(document.body.textContent).toContain("请确认最新版本后再打开");
    expect(document.body.querySelector("#archive-panel")).not.toBeNull();
    ...
    envelope.archives[0]!.savedAt = "2099-01-04T00:00:00.000Z";
    localStorage.setItem(ARCHIVE_STORAGE_KEY, JSON.stringify(envelope));
    ...
    await flushUi();
    expect(document.body.textContent).toContain("档案已在另一标签页更新");
    expect(localStorage.getItem(ARCHIVE_STORAGE_KEY)).toContain("2099-01-04T00:00:00.000Z");
