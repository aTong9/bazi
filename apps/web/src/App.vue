...
function readableSummary(analysis: AnalysisResponse): string {
  ...
  const realityEvidence = !safetyStop && analysisMode.value === "evaluate" ? [
    ...
    ...(analysis.relationship.m5.crossStateEvidence.length ? [
      "## 跨情境核验",
      "",
      ...analysis.relationship.m5.crossStateEvidence.map((evidence) => `- ${CROSS_STATE_LABELS[evidence.state]}：${evidence.note}`),
      "",
    ] : []),
  ] : [];
  ...
  const lines = [
    ...
    `- 报告状态：${STATUS_LABELS[analysis.report.reportStatus] ?? analysis.report.reportStatus}`,
...
function workspaceFingerprint(workspace: ArchiveWorkspaceSnapshot): string {
  ...
  return JSON.stringify([analysisInputFingerprint({
    ...
    primarySubject: workspace.primarySubject,
    hasSecondarySubject: workspace.hasSecondarySubject,
    secondarySubject: workspace.secondarySubject,
    gates: workspace.gates,
    crossState: workspace.crossState,
