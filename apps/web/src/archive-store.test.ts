...
describe("local analysis archive", () => {
  ...
  it("renames the selected archive and records it as the newest backup version", () => {
    ...
    const renamed = renameArchive(first[0]!.id, first[0]!.savedAt, "  小林   长期观察  ", storage, new Date("2026-08-30T03:00:00Z"));
    ...
    expect(() => renameArchive(first[0]!.id, updated[0]!.savedAt, "   ", storage)).toThrow("不能为空");
    expect(loadArchives(storage)[0]?.title).toBe("小林 长期观察");

    expect(() => renameArchive(first[0]!.id, first[0]!.savedAt, "旧标签页名称", storage)).toThrow("已在另一标签页更新");
    expect(loadArchives(storage)[0]?.title).toBe("小林 长期观察");
  });
  ...
  it("rejects unknown, duplicated, oversized, or response-invalid backups without mutating storage", () => {
    ...
    const mismatchedLinkedPillars = structuredClone(current[0]!);
    mismatchedLinkedPillars.workspace.primarySubject.month = "甲子";
    expect(() => importArchiveBackup(JSON.stringify({ ...valid, archives: [mismatchedLinkedPillars] }), storage)).toThrow("档案结构无效");
    const mismatchedSolarSource = structuredClone(current[0]!);
    ...
    const mismatchedCrossStateResult = structuredClone(relationshipArchive(validEvaluate).workspace);
    ...
    mismatchedCrossStateResult.resultInputFingerprint = analysisInputFingerprint(mismatchedCrossStateResult);
    expect(() => importArchiveBackup(serializeArchiveBackup([{ ...validCrossState, workspace: mismatchedCrossStateResult }]), memoryStorage())).toThrow("输入与分析结果不一致");
    ...
    mismatchedObservationResult.result.report.sections.find((section) => section.id === "risk")!.body = `${observedChain.structuralCandidate}（已观察到重复模式）`;
