...
const props = defineProps<{ open: boolean; archives: readonly AnalysisArchive[] }>();
const emit = defineEmits<{ close: []; restore: [archive: AnalysisArchive]; delete: [id: string] }>();
    ...
    <Transition name="archive-panel">
      <div v-if="open" class="archive-overlay" @click.self="emit('close')">
        <aside ref="panel" class="archive-panel" role="dialog" aria-modal="true" aria-labelledby="archive-title">
          ...
          <p class="archive-privacy">档案包含四柱、现实证据与完整分析结果。数据只写入当前浏览器，清理站点数据会同时删除档案。</p>
