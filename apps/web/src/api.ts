...
export function parseAnalysisResponse(value: unknown): AnalysisResponse {
  ...
  const expectedSections = report.reportStatus === "stop" || report.safetyStatus === "safety_stop"
    ...
    : [
        { id: "profile", title: "关系结构候选", body: formatProfileLayers({ attraction: (m1.synthesis as { statements: string[] }).statements, selection: (m2.synthesis as { summary: string[] }).summary, interaction: (m3.synthesis as { statements: string[] }).statements }).join("\n") || "当前没有足够资料形成结构候选。" },
