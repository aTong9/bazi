...
完成分析后可使用“打印 / 存 PDF”生成适合 A4 阅读的看盘报告，或下载可读 Markdown 摘要与完整技术 JSON；也可使用“保存到档案”保存完整工作区，并从顶部“看盘档案”恢复、删除、导出备份或从备份导入。公历辅助产生的 UTC+8 出生时间、历法引擎版本和四柱对应关系会随档案保存，恢复后仍可追溯。档案最多保留 20 份；达到上限后不会静默覆盖旧档案，需先导出或经二次确认删除。数据只写入当前浏览器的本地存储；导出文件未经加密，包含出生时间、四柱与现实证据，需要自行妥善保管。
