...
本仓库是一个本地优先的 M0—M5 八字关系分析工作台。后端使用 Node.js、TypeScript、SQLite 与确定性规则快照，前端使用 Vue 3 与 Vite；界面提供独立 M0 原局结构、单盘关系画像、具体关系现实评估、四柱手动录入、公历时间辅助排盘、双盘结构补充、八道现实闸门、安全停止，以及规则快照/命中摘要和完整 JSON 导出入口。

完成分析后可使用“打印 / 存 PDF”生成适合 A4 阅读的看盘报告，或下载可读 Markdown 摘要与版本化完整看盘 JSON；完整看盘包包含本模式使用的命盘、输入指纹和原始分析响应，关系模式另包含角色口径与现实输入。下载不会自动写入本机档案，也可随后通过“看盘档案”导入恢复。可读摘要会保留公历辅助的 UTC+8 时间、适配器和版本，以及分析 ID 与完整规则快照摘要。三种模式都可使用“保存到档案”保存完整工作区，并从顶部“看盘档案”按名称、命盘称呼或四柱搜索、按看盘类型筛选，以及恢复、重命名、删除、导出单份或全部备份、从备份导入。单份导出可避免把无关看盘资料写入同一文件；导入会先校验并预告新增、更新与跳过数量，确认前不会修改本机档案。自定义档案名称会在后续更新工作区时保留。保存按钮会明确区分首次保存、已保存和观察内容待更新。公历辅助产生的 UTC+8 出生时间、历法引擎版本和四柱对应关系会随档案保存，恢复后仍可追溯；取消“加入另一方命盘”会清除其隐藏资料，旧档案读取时也执行同一迁移。档案最多保留 20 份；达到上限后不会静默覆盖旧档案，需先导出或经二次确认删除。数据只写入当前浏览器的本地存储；导出文件未经加密，包含出生时间、四柱及可能存在的现实证据，需要自行妥善保管。
...
`npm run dev` 会同时启动 API 和 Vite，并在后端或前端源码变化时分别自动重载。浏览器请求 `/health`、`/v1/*` 会由 Vite 代理到本地 API，因此开发环境不需要配置 CORS。
...
截至 2026-08-30，通用测试 runner 共执行 166 个测试用例（Node 核心 82、Web 84）；另有 6 个 Pages 产物/等价性专项测试和 407 项权威开发矩阵。各组口径不同，不合并成一个测试总数。桌面、手机和生产同源路径的浏览器验收记录见 [用户界面与本地开发说明](./docs/development/UI-and-local-development.md)。
