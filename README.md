...
完成分析后可使用“打印 / 存 PDF”生成适合 A4 阅读的看盘报告，或下载可读 Markdown 摘要与版本化完整看盘 JSON；完整看盘包同时包含命盘、角色口径、现实输入、输入指纹和原始分析响应，但不会自动写入本机档案，也可随后通过“看盘档案”导入恢复。可读摘要会保留公历辅助的 UTC+8 时间、适配器和版本，以及分析 ID 与完整规则快照摘要。也可使用“保存到档案”保存完整工作区，并从顶部“看盘档案”按名称、命盘称呼或四柱搜索，以及恢复、重命名、删除、导出单份或全部备份、从备份导入。单份导出可避免把无关看盘资料写入同一文件；导入会先校验并预告新增、更新与跳过数量，确认前不会修改本机档案。自定义档案名称会在后续更新工作区时保留。保存按钮会明确区分首次保存、已保存和观察内容待更新。公历辅助产生的 UTC+8 出生时间、历法引擎版本和四柱对应关系会随档案保存，恢复后仍可追溯；取消“加入另一方命盘”会清除其隐藏资料，旧档案读取时也执行同一迁移。档案最多保留 20 份；达到上限后不会静默覆盖旧档案，需先导出或经二次确认删除。数据只写入当前浏览器的本地存储；导出文件未经加密，包含出生时间、四柱与现实证据，需要自行妥善保管。
