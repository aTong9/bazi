...
test("all 150 V1.9 M20 fixtures produce explicit execution or governance records", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(records.filter((record) => record.executionStatus === "executed").length, 150);
      assert.equal(records.filter((record) => record.executionStatus === "review_required").length, 0);
