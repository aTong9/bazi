...
export function analyzeM3(input: { m02: M02Result; m09: M09Result; m10: M10Result; rules: RelationshipRuleCatalog }): M3Result {
  ...
  const synthesisCandidates = new Map<ChannelKey, string>();
  const channelEntries = (Object.entries(MODULES) as Array<[ChannelKey, string]>).map(([key, moduleId]): [ChannelKey, M3Channel] => {
    ...
    const anchored = records.filter((record) => !["定义", "边界"].includes(field(record, "rule_type")) && (field(record, "main_signal").includes(anchorTenGod) || field(record, "required_conditions").includes(`日支${anchorTenGod}`))).slice(0, 12);
  ...
  const synthesisStatements = unique(primaryChannels.flatMap((key) => synthesisCandidates.get(key) ?? []));
