...
export function projectM19(input: {
  ...
  set(fields, "climate_problem_and_urgency", result(input.m16.problems.filter((problem) => problem.type === "climate"), "conditional", "medium_low", [...input.m13.matchedRuleIds, ...input.m16.matchedRuleIds], input.m16.problems.some((problem) => problem.type === "climate") ? [] : ["CLIMATE_IS_BACKGROUND_NOT_PRIMARY_DISEASE"]));
  set(fields, "climate_candidates_and_conflicts", result({ temperature: input.m13.temperature.candidateElements, humidity: input.m13.humidity.candidateElements, conflicts: input.m13.conflicts }, "conditional", "medium_low", [...input.m13.matchedRuleIds, ...input.m18.matchedRuleIds], []));
  ...
  set(fields, "pattern_evidence_and_level", result(input.m15.evaluations.map((item) => ({ name: item.name, formation: item.formation, purity: item.purity, conditions: item.conditions })), "conditional", "medium_low", input.m15.matchedRuleIds, []));
  ...
  set(fields, "pattern_rescue_and_alternatives", result(input.m15.evaluations.map((item) => ({ pattern: item.name, rescue: item.rescue })), "conditional", "medium_low", [...input.m15.matchedRuleIds, ...input.m18.matchedRuleIds], []));
