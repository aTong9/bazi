...
export interface M20ExecutionRecord {
  ...
  readonly targetRulesExist: boolean; readonly targetRulesMatched: readonly string[]; readonly issues: readonly string[];
}
...
export async function executeAllM20Fixtures(options: { repositoryRoot: string; catalog: CatalogSnapshot }): Promise<readonly M20ExecutionRecord[]> {
  ...
  return Object.freeze(rows.map((row): M20ExecutionRecord => {
    ...
    if (row.fields["测试结果"] === "待复核") return record(row.id, row.fields, targetRuleIds, "review_required", targetRulesExist, [], ["SOURCE_FIXTURE_REQUIRES_HUMAN_OR_GOVERNANCE_REVIEW"]);
    ...
    if (!pillars) {
      ...
      return record(row.id, row.fields, targetRuleIds, qualityGate ? "quality_gate" : "failed", targetRulesExist, [], qualityGate ? ["INVALID_INPUT_EXPECTED_BY_FIXTURE"] : ["FOUR_PILLARS_NOT_PARSEABLE"]);
    }
    ...
    if (!result.ok) return record(row.id, row.fields, targetRuleIds, "failed", targetRulesExist, [], result.issues.map((issue) => issue.code));
    ...
    return record(row.id, row.fields, targetRuleIds, issues.length ? "failed" : "executed", targetRulesExist, matched, issues);
  }));
}

function record(testId: string, fields: Readonly<Record<string, string>>, targetRuleIds: readonly string[], executionStatus: M20ExecutionRecord["executionStatus"], targetRulesExist: boolean, targetRulesMatched: readonly string[], issues: readonly string[]): M20ExecutionRecord {
  return Object.freeze({ testId, sourceStatus: fields["测试结果"] ?? "", targetModule: fields["目标模块"] ?? "", targetRuleIds: Object.freeze(targetRuleIds), executionStatus, targetRulesExist, targetRulesMatched: Object.freeze(targetRulesMatched), issues: Object.freeze(issues) });
}
